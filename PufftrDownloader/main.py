...
from .log import UnLogger, log_warn, log_err
from .dataClass import *
from .style import *
from . import log

WORKINGDIR = Path(__file__).parent
if not (WORKINGDIR/"conf.json").exists():
    print("conf.json must be in root folder of module")
    exit(1)
with open(WORKINGDIR/"conf.json") as f:
    conf = json.load(f)
    
WORKINGDIR = WORKINGDIR if conf["WORKINGDIR"] == None else Path(conf["WORKINGDIR"])
OUTPUTDIR = Path(os.getcwd()).expanduser()/"out" if conf["OUTPUTDIR"] == None else Path(conf["OUTPUTDIR"])
...
log.WARN_LOG_FILE = WARN_LOG_FILE
...
def resolve_player_location(page:Page, video:VideoData, page_url:str, episode_url:str) -> bool:
    try:
        resp = page.request.get(video.site_url, max_redirects=0, headers={"Referer": page_url,"Origin": BASE_URL})
    except Exception as e:
